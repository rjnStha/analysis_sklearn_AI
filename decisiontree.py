...
def load_data():
  ...
  with open('Cumulative.csv', newline='') as csvfile:
    ...
    for row in spamreader:
      #subdata not first generation
      if row[5] != '' and row[6] != '' and row[8] != '' and row[9] != '' and row[1] != 'M':
        #different feature and label for different hypothesis
        #Cumulative GPA
        #Grade in Fundamentals Course
        #Grade in Systems Course
        #Semester taken to grad 
        #Grade in Intro Course
        
        if row[9] == '2' : major = 0
        else : major = 1
        sat_math = int(row[5])
        high_gpa = float(row[6])
        cuml_gpa = float(row[8])
        
        
        data.append([sat_math, high_gpa, cuml_gpa, major])
       
        count = count+1
      if count == 1000 : break
    print("total sample size ", count)
  return random.sample(data,250)
...
non_goal_attr1 = 'sat_math'
non_goal_attr2 = 'high_gpa'
non_goal_attr3 = 'cuml_gpa'
goal_attr1 = '0'
goal_attr2 = '1'
goal_attr3 = '3'
goal_attr4 = '4'
...
print("forest accuracy score: ", accuracy_score(y_test, y_predict))
